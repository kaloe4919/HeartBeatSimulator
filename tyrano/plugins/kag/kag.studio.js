tyrano.plugin.kag.studio = {
  ...
  map_watch: {},
  init: function () {
    if (-1 != window.navigator.userAgent.indexOf("TyranoStudio")) {
      try {
      ...
      j_reload_button.draggable({ scroll: !1, stop: (e, ui) => {} });
        ...
        j_img.draggable({
          ...
          stop: (e, ui) => {
            let left = Math.floor(ui.position.left),
              top = Math.floor(ui.position.top);
  ...
  cutTyranoScript: function (str) {
    for (
      var array_s = TYRANO.kag.parser.parseScenario(str).array_s, i = 0;
      i < array_s.length;
      i++
    ) {
      var tag = array_s[i];

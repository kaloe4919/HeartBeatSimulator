  ...
  showMessage: function (message_str, pm, isVertical) {
    ...
    !(function (jtext) {
      ...
      for (var append_str = "", i = 0; i < message_str.length; i++) {
        ...
        if (" " == c) append_str += "<span style='opacity:0'>" + c + "</span>";
        else {
          if (1 == that.kag.stat.mark) {
            c =
              "<mark style='" + that.kag.stat.style_mark + "'>" + c + "</mark>";
          } else 2 == that.kag.stat.mark && (that.kag.stat.mark = 0);
          append_str += flag_in_block
      ...
      if (that.kag.stat.fuki.active) {
        ...
        let chara_fuki = {},
          is_chara_show = !1;
        if ("" == chara_name) is_chara_show = !1;
        else {
          let original_name = chara_name;
          that.kag.stat.jcharas[chara_name] &&
          ...
          if (
            void 0 !== chara_obj &&
            chara_obj.get(0) &&
            "true" == that.kag.stat.charas[original_name].fuki.enable
          ) {
            is_chara_show = !0;
            ...
            if ("true" == that.kag.stat.vertical) {
            ...
            height =
              ...
              20;
            (j_outer_message = that.kag.getMessageOuterLayer()).css(
              "width",
              width,
            );
            ...
            let fuki_left = chara_fuki.left,
              ...
              origin_width = that.kag.stat.charas[original_name].origin_width,
              origin_height = that.kag.stat.charas[original_name].origin_height;
            fuki_left *= chara_width / origin_width;
            fuki_top *= chara_height / origin_height;
          ...
          } else is_chara_show = !1;
        }
        if (0 == is_chara_show) {
          $(".tyrano_base").find(".chara_name_area").hide();
          let others_style = that.kag.stat.fuki.others_style,
            def_style = that.kag.stat.fuki.def_style,
...
tyrano.plugin.kag.tag.position = {
  ...
  start: function (pm) {
    ...
    "" != pm.opacity && (new_style.opacity = $.convertOpacity(pm.opacity));
  ...
  setEvent: function (j_button, pm) {
    ...
    !(function () {
      var _target = pm.target,
        ...
        button_clicked = !1,
        parse_img_url = function (src) {
          return $.isHTTP(src) ? src : "./data/" + _pm.folder + "/" + src;
        };
      j_button.hover(
        function () {
          "" != _pm.enterse &&

  ...
  start: function (pm) {
    ...
    if ("" != pm.bgimage)
      if ("transparent" == pm.bgimage) {
        ...
        var img_url = "";
        img_url = $.isHTTP(pm.bgimage)
          ? pm.bgimage
          : "./data/bgimage/" + pm.bgimage;
...
tyrano.plugin.kag.tag.if = {
  ...
  start: function (pm) {
    if (this.kag.embScript(pm.exp)) {
      this.kag.pushStack("if", { bool: !0, deep: pm.deep_if });
      this.kag.ftag.nextOrder();
    } else {
      this.kag.pushStack("if", { bool: !1, deep: pm.deep_if });
      for (var i = 0; i < 2e3; i++) {
        if (
          1 ==
          this.kag.ftag.nextOrderWithTag({ else: "", elsif: "", endif: "" })
        )
          break;
      }
...
tyrano.plugin.kag.tag.elsif = {
  ...
  start: function (pm) {
    if (0 == this.kag.getStack("if").bool && this.kag.embScript(pm.exp)) {
      this.kag.setStack("if", { bool: !0, deep: pm.deep_if });
      this.kag.ftag.nextOrder();
    } else {
      for (var i = 0; i < 2e3; i++) {
        if (
...
tyrano.plugin.kag.tag.else = {
  ...
  start: function (pm) {
    if (0 == this.kag.getStack("if").bool) {
      ...
      for (var i = 0; i < 2e3; i++) {
        if (1 == this.kag.ftag.nextOrderWithTag({ endif: "" })) break;
      }
      i > 1900 && this.kag.error("If文に誤りがあります");
    }
  },
...
tyrano.plugin.kag.tag.call = {
  ...
  start: function (pm) {
    ...
    null == pm.target && null != pm.storage
...
tyrano.plugin.kag.tag.return = {
  start: function () {
    ...
    if (
      pm.caller &&
      pm.caller.storage &&
      ("make.ks" == pm.caller.storage ||
        pm.caller.storage == this.kag.stat.resizecall.storage) &&
      1 == this.kag.tmp.loading_make_ref
    ) {
...
tyrano.plugin.kag.tag.macro = {
  ...
  start: function (pm) {
    ...
    for (var i = 0; i < 2e3; i++) {
      if (1 == this.kag.ftag.nextOrderWithTag({ endmacro: "" })) break;
    }
...
tyrano.plugin.kag.tag.ignore = {
  ...
  start: function (pm) {
    if (this.kag.embScript(pm.exp)) {
      for (var i = 0; i < 2e3; i++) {
        if (1 == this.kag.ftag.nextOrderWithTag({ endignore: "" })) break;
      }
...
tyrano.plugin.kag.tag.awakegame = {
  vital: [],
  pm: { variable_over: "true", bgm_over: "true" },
  start: function (pm) {
    if (null == this.kag.tmp.sleep_game) this.kag.ftag.nextOrder();
      ...
      var options = { bgm_over: pm.bgm_over };
...
tyrano.plugin.kag.tag.dialog = {
  ...
  start: function (pm) {
    ...
    if ("confirm" == pm.type)
      ...
      alertify.set({
        buttonFocus: "none",
        labels: { ok: pm.label_ok, cancel: pm.label_cancel },
      });
...
tyrano.plugin.kag.tag.check_web_patch = {
  ...
  checkPatch: function (obj, pm) {
    ...
    parseFloat(this.kag.variable.sf._patch_version) < parseFloat(obj.version)
      ? $.confirm(

tyrano.plugin.kag.tag.bgcamera = {
  ...
  start: function (pm) {
    ...
    if ("" != pm.height) video.style.height = pm.height + "px";
    else if ("true" == pm.fit) {
      if (
        parseInt(this.kag.config.scWidth) > parseInt(this.kag.config.scHeight)
      ) {
        video.style.height = "";
        video.style.width = "100%";
      } else {
    ...
    "back" == pm.mode
      ...
      : "front" == pm.mode && (opt.video = { facingMode: "user" });
    let j_video = $(video);
  ...
  checkPicture: function (video, pm) {
    ...
    var scWidth = parseInt(this.kag.config.scWidth) / 4,
      scHeight = parseInt(this.kag.config.scHeight) / 4,
    ...
    if (code) {
      ...
      if (-1 != url.indexOf("http")) {
        if ("all" == mode || "define" == mode || "web" == mode)

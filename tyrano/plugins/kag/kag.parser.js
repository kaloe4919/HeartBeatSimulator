tyrano.plugin.kag.parser = {
  ...
  compileConfig: function (text_str) {
    for (
      var error_str = "",
        map_config = {},
        array_config = text_str.split("\n"),
        i = 0;
      i < array_config.length;
      i++
    )
      try {
        ...
        if ("" != line_str && ";" === line_str.substr(0, 1)) {
          ...
          var tmp = (line_str = $.replaceAll(line_str, '"', "")).split("="),
            key = $.trim(tmp[0]),
            val = $.trim(tmp[1]);
          map_config[key] = val;
        }
  ...
  parseScenario: function (text_str) {
    for (
      ...
      var line_str = $.trim(array_row[i]),
        first_char = line_str.substr(0, 1);
      -1 != line_str.indexOf("endscript") && (this.flag_script = !1);
      if (!0 === flag_comment && "*/" === line_str) flag_comment = !1;
      else if ("/*" === line_str) flag_comment = !0;
      else if (1 == flag_comment || ";" === first_char);
      else if ("#" === first_char) {
        ...
        var label_key,
          label_tmp = line_str.substr(1, line_str.length).split("|"),
          label_val = "";
        ...
        map_label[label_obj.pm.label_name]
          ? this.kag.warning(
              "Warning line:" +
                i +
                " " +
        ...
        for (
          ...
          var c = array_char[j];
          if (!0 === flag_tag)
            if ("]" === c && 0 == this.flag_script)
              if (0 == --num_kakko) {
                flag_tag = !1;
                array_s.push(this.makeTag(tag_str, i));
                tag_str = "";
              } else tag_str += c;
            else if ("[" === c && 0 == this.flag_script) {
              num_kakko++;
              tag_str += c;
            } else tag_str += c;
          else if (!1 === flag_tag && "[" === c && 0 == this.flag_script) {
            num_kakko++;
            if ("" != text) {
              text_obj = {
                line: i,

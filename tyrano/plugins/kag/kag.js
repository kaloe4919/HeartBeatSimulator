tyrano.plugin.kag = {
  ...
  applyPatch: function (patch_path, flag_reload, call_back) {
    ...
    if (fs.existsSync(patch_path)) {
      ...
      if ("asar" != unzip_path) {
        ...
        var AdmZip;
        new (AdmZip = require("adm-zip"))(patch_path).extractAllTo(
          _path.resolve(out_path + "update_tmp/"),
          !0,
        );
  ...
  init_game: function () {
    ...
    for (
      var num_message_layer = parseInt(this.config.numMessageLayers), i = 1;
      i < num_message_layer;
      i++
    ) {
    ...
    for (
      var fore_layer_num = parseInt(this.config.numCharacterLayers), i = 0;
      i < fore_layer_num;
      i++
    ) {
  ...
  readyAudio: function () {
    this.tmp.ready_audio = !0;
    if ($.isNeedClickAudio()) {
      new Howl({ src: "./tyrano/audio/silent.mp3", volume: 0.1 }).play();
    }
  },
  ...
  preloadAll: function (storage, callbk) {
    if ("object" == typeof storage && storage.length >= 0) {
      ...
      for (var sum = 0, i = 0; i < storage.length; i++)
        this.kag.preload(storage[i], function () {
          sum++;
  ...
  error: function (str) {
    if ("true" == this.kag.config["debugMenu.visible"]) {
      var err =
        ...
        ":" +
        (parseInt(this.kag.stat.current_line) + 1) +
        "行目:" +
        str;
      $.error_message(err);
    }
